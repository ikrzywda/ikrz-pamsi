...
int format_message(char *buffer, LogLevel level, const char *message) {
  static char *LOG_LEVEL_STRINGS[] = {"DEBUG", "INFO", "WARN", "ERROR",
                                      "FATAL"};
  static char *LOG_LEVEL_COLORS[] = {"\033[0;36m", "\033[0;32m", "\033[0;33m",
                                     "\033[0;31m", "\033[0;35m"};
  ...
  snprintf(log_level_tag, LOG_LEVEL_TAG_BUFFER_CAPACITY, "%s%s%s",
           LOG_LEVEL_COLORS[level], LOG_LEVEL_STRINGS[level], RESET_COLOR);
  snprintf(buffer, LOG_MESSAGE_BUFFER_CAPACITY, "[%s] [%s] %s\n", time_buffer,
           log_level_tag, message);
  return SUCCESS;
}
...
int log_message(Logger *logger, LogLevel level, const char *message) {
  ...
  int status_code;
  if ((status_code = format_message(log_message_buffer, level, message)) !=
      SUCCESS) {
